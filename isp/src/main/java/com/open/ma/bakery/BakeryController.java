...
import com.open.ma.bakery.service.BakeryVO;
 ...
 */
@Controller
public class BakeryController {  /** @Controller로 등록된 클래스 파일에 대한 bean 자동생성*/
    ...
    /** Program ID **/
    private final static String PROGRAM_ID = "Bakery"; 
    /** DB쿼리문 실행할 때 필요한 id값 세팅-- sql.xml 파일 mapper namespace 동일해야함!!  */

    /** folderPath **/
    private final static String folderPath = "/ma/pd/bakery/"; /** 상위 폴더 경로를 folderPath로 지정*/
	...
	public String addList(@ModelAttribute("searchVO") CmmnDefaultVO searchVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<BakeryVO> resultList = (List<BakeryVO>) cmmnService.selectList(searchVO, PROGRAM_ID );
	...
	public String view(@ModelAttribute("searchVO") BakeryVO searchVO, Model model, HttpServletRequest request) throws Exception {
		
		/* 게시판 상세정보 */
		BakeryVO bakeryVO = new BakeryVO();
		bakeryVO = (BakeryVO) cmmnService.selectContents(searchVO, PROGRAM_ID );
		model.addAttribute("bakeryVO", bakeryVO);
	...
	public String form(@ModelAttribute("searchVO") BakeryVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
	...
	public String proc(@ModelAttribute("searchVO") BakeryVO searchVO, Model model, SessionStatus status,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		if(procType != null){
			...
			if(procType.equals("update")){
				...
				model.addAttribute("pName", "bkSeq");	
				model.addAttribute("pValue", searchVO.getBkSeq());
	...
	public String smCateList(@ModelAttribute("searchVO") BakeryVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
		
		String[] bkCategoryList = {"카테고리선택", "베이커리", "디저트", "케이크", "샐러드", "잼" };
		model.addAttribute("bkCategoryList", bkCategoryList);
