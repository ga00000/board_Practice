...
import com.open.ma.noodle.service.NoodleVO;
 ...
 */
@Controller
public class NoodleController {  /** @Controller로 등록된 클래스 파일에 대한 bean 자동생성*/
    ...
    /** Program ID **/
    private final static String PROGRAM_ID = "Noodle"; 
    /** DB쿼리문 실행할 때 필요한 id값 세팅-- sql.xml 파일 mapper namespace 동일해야함!!  */

    /** folderPath **/
    private final static String folderPath = "/ma/pd/noodle/"; /** 상위 폴더 경로를 folderPath로 지정*/
	...
	public String addList(@ModelAttribute("searchVO") CmmnDefaultVO searchVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<NoodleVO> resultList = (List<NoodleVO>) cmmnService.selectList(searchVO, PROGRAM_ID );
	...
	public String view(@ModelAttribute("searchVO") NoodleVO searchVO, Model model, HttpServletRequest request) throws Exception {
		
		/* 게시판 상세정보 */
		NoodleVO noodleVO = new NoodleVO();
		noodleVO = (NoodleVO) cmmnService.selectContents(searchVO, PROGRAM_ID );
		model.addAttribute("noodleVO", noodleVO);
	...
	public String form(@ModelAttribute("searchVO") NoodleVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
	...
	public String proc(@ModelAttribute("searchVO") NoodleVO searchVO, Model model, SessionStatus status,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		if(procType != null){
			...
			if(procType.equals("update")){
				...
				model.addAttribute("pName", "ndSeq");	
				model.addAttribute("pValue", searchVO.getNdSeq());
	...
	public String ndCateList(@ModelAttribute("searchVO") NoodleVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
		
		String[] ndCategoryList = {"카테고리선택", "베이커리", "디저트", "케이크", "샐러드", "잼" };
		model.addAttribute("ndCategoryList", ndCategoryList);
