...
import com.open.ma.drink.service.DrinkVO;
 ...
 */
@Controller
public class DrinkController {  /** @Controller로 등록된 클래스 파일에 대한 bean 자동생성*/
    ...
    /** Program ID **/
    private final static String PROGRAM_ID = "Drink"; 
    /** DB쿼리문 실행할 때 필요한 id값 세팅-- sql.xml 파일 mapper namespace 동일해야함!!  */

    /** folderPath **/
    private final static String folderPath = "/ma/pd/drink/"; /** 상위 폴더 경로를 folderPath로 지정*/
	...
	public String addList(@ModelAttribute("searchVO") CmmnDefaultVO searchVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<DrinkVO> resultList = (List<DrinkVO>) cmmnService.selectList(searchVO, PROGRAM_ID );
	...
	public String view(@ModelAttribute("searchVO") DrinkVO searchVO, Model model, HttpServletRequest request) throws Exception {
		
		/* 게시판 상세정보 */
		DrinkVO drinkVO = new DrinkVO();
		drinkVO = (DrinkVO) cmmnService.selectContents(searchVO, PROGRAM_ID );
	...
	public String form(@ModelAttribute("searchVO") DrinkVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
	...
	public String proc(@ModelAttribute("searchVO") DrinkVO searchVO, Model model, SessionStatus status,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		if(procType != null){
			...
			if(procType.equals("update")){
				...
				model.addAttribute("pName", "drSeq");	
				model.addAttribute("pValue", searchVO.getDrSeq());
