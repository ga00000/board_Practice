...
import com.open.ma.product.service.ProductVO;
 ...
 */
@Controller
public class ProductController {  /** @Controller로 등록된 클래스 파일에 대한 bean 자동생성*/
    ...
    /** Program ID **/
    private final static String PROGRAM_ID = "Product"; 
    /** DB쿼리문 실행할 때 필요한 id값 세팅-- sql.xml 파일 mapper namespace 동일해야함!!  */

    /** folderPath **/
    private final static String folderPath = "/ma/pd/product/"; /** 상위 폴더 경로를 folderPath로 지정*/
	...
	public String addList(@ModelAttribute("searchVO") CmmnDefaultVO searchVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<ProductVO> resultList = (List<ProductVO>) cmmnService.selectList(searchVO, PROGRAM_ID );
		model.addAttribute("resultList", resultList); /** "아 model로 넘김 */
	...
	public String view(@ModelAttribute("searchVO") ProductVO searchVO, Model model, HttpServletRequest request) throws Exception {
		
		/* 게시판 상세정보 */
		ProductVO productVO = new ProductVO();
		productVO = (ProductVO) cmmnService.selectContents(searchVO, PROGRAM_ID );
	...
	public String form(@ModelAttribute("searchVO") ProductVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		productVO.setSearchVO(searchVO);
		model.addAttribute("productVO", productVO);
	...
	public String proc(@ModelAttribute("searchVO") ProductVO searchVO, Model model, SessionStatus status,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		if(procType != null){
			...
			if(procType.equals("update")){
				...
				model.addAttribute("pName", "PdSeq");	
				model.addAttribute("pValue", searchVO.getPdSeq());
