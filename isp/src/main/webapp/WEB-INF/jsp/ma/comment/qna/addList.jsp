			...
			<c:choose>
				<c:when test="${fn:length(resultList) gt 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
								<tr class="cursor" onclick="fncPageBoard('view','view.do','${result.qaSeq}','qaSeq')">
								<td>${paginationInfo.totalRecordCount+1- ((searchVO.pageIndex-1) * searchVO.pageUnit + status.count)}</td>
								<td> ${result.qaAnswerYn eq "N" ? '부' : '여'}</td>
								<td class="subject"><span class="lengthCut" >${result.qaTitle }</span></td>
								<td>${result.qaRgstId }</td>
								<td>${result.qaRgstDt }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
