			...
			<c:choose>
				<c:when test="${fn:length(resultList) gt 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor" onclick="fncPageBoard('view','view.do','${result.bkSeq}','bkSeq')">
							<td>
							...
							<td class="subject  lengthCut">${util:cutText(result.bkTitle,100,'...') }</td>
							<td>${result.bkRgstId }</td>
							<td>${result.bkRgstDt }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
