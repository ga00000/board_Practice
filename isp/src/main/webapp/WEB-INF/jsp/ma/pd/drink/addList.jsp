			...
			<c:choose>
				<c:when test="${fn:length(resultList) gt 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor" onclick="fncPageBoard('view','view.do','${result.drSeq}','drSeq')">
							<td>
							...
							<td class="subject"><span class=" lengthCut">${result.drTitle }</span></td>
							<td>${result.drRgstId }</td>
							<td>${result.drRgstDt }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
