			...
			<c:choose>
				<c:when test="${fn:length(resultList) gt 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor" onclick="fncPageBoard('view','view.do','${result.smSeq}','smSeq')">
							<td>
							${paginationInfo.totalRecordCount+1- ((searchVO.pageIndex-1) * searchVO.pageUnit + status.count)}
							<%-- 페이지당 게시물수를 차감해야하는데, 페이지수로 차감하니 목록번호 오류발생
								${status.count} 는 1부터의 순서--%>
							</td>
							<td class="subject" >${util:cutText(result.smTitle,20,'...') }</td>
							<td>${result.smRgstId }</td>
							<td>${result.smRgstDt }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
