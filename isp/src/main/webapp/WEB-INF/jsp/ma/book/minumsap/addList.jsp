			...
			<c:choose>
				<c:when test="${fn:length(resultList) gt 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor" onclick="fncPageBoard('view','view.do','${result.miSeq}','miSeq')">
							<td>
							...
							<td class="subject"><span class=" lengthCut">${result.miTitle }</span></td>
							<td>${result.miRgstId }</td>
							<td>${result.miRgstDt }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
