			...
			<c:choose>
				<c:when test="${fn:length(resultList) gt 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor" onclick="fncPageBoard('view','view.do','${result.yesSeq}','yesSeq')">
							<td>
							${paginationInfo.totalRecordCount+1- ((searchVO.pageIndex-1) * searchVO.pageUnit + status.count)}
							</td>
							<td class="subject"><span class=" lengthCut">${result.yesTitle }</span></td>
							<td>${result.yesRgstId }</td>
							<td>${result.yesRgstDt }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
